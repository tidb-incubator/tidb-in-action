...
- DM-master，负责管理整个 DM 集群，以及调度同步任务
- DM-woker，执行具体的同步任务
  - 一个 DM-worker 注册为一个上游 MySQL 或 MariaDB 实例的 Slave
...
- dmctl，控制 DM 集群的命令行工具，连接 DM-Master 管理整个集群
...
- [DM 集群的部署启动文档](https://pingcap.com/docs-cn/stable/how-to/deploy/data-migration-with-ansible/)，部署过程与 TiDB 集群的部署启动高度相似，inventory.ini 需要注意以下几点
...
- 查看详细任务状态，正常状态 result 为 true，DM-worker 内的 Binlog 位置一致，同步过程中也会展示同步百分比
