# 背景
大量应用程序通常使用唯一的标识符来确认一行记录。在传统的解决方案中，普遍依赖数据库的序列对象（sequence）来生成唯一标识中的数字部分。TiDB 目前版本（v3.0）还不支持序列，据悉 TiDB 将在 v4.0 版本正式提供序列功能。

互联网应用需要处理的的数据量往往呈现爆发式增长，这使得应用需要为大量的数据和消息生成唯一标识。即使有了序列功能，数据库也可能会由于高并发的序列分配请求而遇到性能瓶颈。下面介绍两种高性能的序列号生成方案。
...
在分布式系统中，高并发场景下是否可以稳定快速拿到一个唯一序列号至关重要。下面介绍一个生成唯一序列号的方式；
