...
# 方案二：号段分配方案
本方案需要一张序列号生成表，每个序列使用一行数据来控制，这张表需要具有序列名称、序列最大值、序列获取步长（step）等字段，应用程序每次按配置好的步长来获取一批序列号，并同时更新该序列最大值，在应用程序内存中完成最终的序列号加工及分配。在预期并发变高时，可以通过调大序列获取步长的方式来降低这行记录上的更新并发。
