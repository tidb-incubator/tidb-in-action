# 背景
应用程序通常使用唯一的标识符来确认一行记录。在传统解决方案中，普遍依赖数据库的序列对象（sequence）来生成唯一标识中的数字部分。TiDB 目前版本（v3.0）还不支持序列，据悉将在 v4.0 版本正式提供该功能。互联网应用需要处理的数据量巨大且往往呈现爆发式增长趋势，这使得应用需要为大量的数据和消息生成唯一标识。即使有了序列功能，数据库也可能会由于高并发的序列分配请求而出现性能瓶颈。
