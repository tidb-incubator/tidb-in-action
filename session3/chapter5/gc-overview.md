...
TiDB 的事务的实现采用了 MVCC（多版本并发控制）机制，当需要对旧的数据修改时，旧的数据不会被替换掉，而是与新写入的数据同时保留，并通过时间戳来区分版本。GC 的任务便是清理不再需要的旧数据。

## 整体流程
一个 TiDB 集群中会有一个 TiDB 实例被选举为 GC leader，GC 的运行由 GC leader 来控制。

GC 会被定期触发，默认情况下每 10 分钟一次。每次 GC 时，TiDB 会先计算一个称为 safe point 的时间戳（默认为当前时间减去 10 分钟），然后 TiDB 在保证 safe point 之后的快照全部拥有正确数据的前提下，将删除更早的过期数据。具体而言，分为以下三个步骤：
